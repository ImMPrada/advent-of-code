module Year2024
  module Day16
    module Renderable # rubocop:disable Metrics/Metrics/ModuleLength
      def render(title = nil, path = nil)
      ...
      def generate_stats_html(path)
        ...
        <<~HTML
          <div class="stats">
            Path length: #{path.size} steps<br>
            Total cost: #{path.last[3]} points
          </div>
        HTML
      end

      def generate_cells_html(path)
        path_cells = path ? path_to_set(path) : Set.new

        cells.to_a.map do |row|
          row.map do |cell|
            cell_class = cell_class_for(cell, path_cells)
            direction = direction_for_cell(cell, path)

            # Debug cell info
            puts "Found path cell at (#{cell.x}, #{cell.y})" if path_cells.include?([cell.x, cell.y])
      ...
      def cell_class_for(cell, path_cells)
        if cell == starting_cell
          ...
          'cell wall'
        elsif path_cells.include?([cell.x, cell.y])
          'cell path'
        else
          'cell'
        end
      ...
      def direction_for_cell(cell, path)
        return nil unless path

        path.find { |x, y, _dir, _cost| x == cell.x && y == cell.y }&.at(2)
      end
